...
| `Respond` | flag | The recipient must act: a reply is expected, or a required task is requested. Optional surveys and similar do not count. |
...
recommend when the options may not cover every input. `Respond` and `Human`
...
A message gets at most 2 category labels. Flags (`Respond`, `Human`) are
...
example, a booking confirmation that needs a reply gets `Respond`,
`Booking`, and `Receipt`.
...
```json
{
  ...
  "flags": {
    "action": { "label": "Respond", "threshold": 0.7, "instructions": "...", "criteria": { "...": "..." } },
